...
export default function Manufacture(props) {
    ...
    React.useEffect(() => {
        navigator.geolocation.getCurrentPosition(position => {
            setManuForm(prev => ({
                ...prev,
                manufacturerLatitude: String(position.coords.latitude),
                manufacturerLongitude: String(position.coords.longitude)
            }));
        })
    }, []);
    ...
    const handleSubmitManufacturerForm = async () => {
        ...
        if (manuForm.manufacturerName !== "" && manuForm.manufacturerDetails !== "" && manuForm.manufacturerLongitude !== "" && manuForm.manufacturerLatitude !== "" && manuForm.productName !== "" && manuForm.productCode !== 0 && manuForm.productPrice !== 0 && manuForm.productCategory !== "") {
            setfvalid(false);
            await supplyChainContract.methods.manufactureProduct(manuForm.manufacturerName, manuForm.manufacturerDetails, manuForm.manufacturerLatitude, manuForm.manufacturerLongitude, manuForm.productName, parseInt(manuForm.productCode), parseInt(manuForm.productPrice), manuForm.productCategory).send({ from: roles.manufacturer, gas: 999999 })
                .then((receipt) => {
                    console.log(receipt);
                    // handleSetTxhash(receipt);
                })
